...
class AmbientContextSettingsTests(unittest.TestCase):
    def test_defaults_to_london_disabled(self) -> None:
        ...
        self.assertFalse(state.sky_enabled)
    ...
    def test_update_persists_and_restores_across_instances(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            first.update(
                enabled=True, timezone="Asia/Tokyo", location="Tokyo, JP", weather_enabled=True, sky_enabled=True
            )
            ...
            self.assertTrue(restored.sky_enabled)
