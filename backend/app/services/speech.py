...
# Schema types historically re-exported from this module (tts_worker, tests, and
# other callers import them from app.services.speech). Kept as re-exports so the
# adapter extraction stays import-compatible.
from app.schemas.session import (  # noqa: F401  (re-exported for callers)
    AudioFormatMetadata,
    SpeechLipSyncDebug,
...
# Public speech-audio artifact projection (extracted). Re-exported so existing
# `app.services.speech.X` imports (operator_routes, turns, tts_worker) keep
# resolving; resolve_session_speech_artifact_path stays here (it needs the
# stream constant + event store).
from app.services.speech_artifacts import (  # noqa: F401  (re-exported for callers)
    PUBLIC_SESSION_SPEECH_ARTIFACT_PREFIX,
    _normalize_audio_reference,
    _resolve_public_speech_audio_artifact_path,
    build_public_speech_audio_reference,
    project_public_session_event,
    project_public_speech_lifecycle_envelope,
    project_public_speech_lifecycle_snapshot,
)
# Provider adapter layer (extracted). The registry + lifecycle fallback use the
# adapters/requests directly; the rest are re-exported so existing
# `app.services.speech.X` imports (turns, tts_worker, tts_engines, tests) resolve.
from app.services.speech_adapters import (  # noqa: F401  (re-exported for callers)
