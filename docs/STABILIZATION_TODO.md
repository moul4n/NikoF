...
- [ ] **Status refresh wedges the dashboard during backend downtime** *(confirmed acute 2026-06-10)* — while the backend is restarting (~15s), `Build-ManagerStatus`'s sequential `Invoke-JsonGet` calls each hit their 2s timeout, but the browser polls `/api/status` every 3s, so requests pile up faster than the single-threaded HttpListener drains them → the dashboard wedges and answers nothing (even `/health`). Parallelize/cache the status build, shorten timeouts, and/or serve a cached snapshot so a slow backend can't saturate the listener. (Part of the existing 1A status-refresh item.)
