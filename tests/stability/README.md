...
- `animation-contract-boundaries`: snapshots the backend animation dataclass and service signatures directly from source, plus the staged shared animation registry and sidecar promotion flags, so contract drift and accidental promotion of staged DSL sidecars fail without inventing unimplemented runtime behavior.
- `contracts-validation`: snapshots the current contract validator command output and exit code.
- `bootstrap-prerequisites`: snapshots the declared bootstrap tooling contract from `bootstrap.targets.json` plus missing provider payload expectations, Faster-Whisper Medium and GPT-SoVITS acceptance targets and blockers, and provider install guidance inside a harness-owned sandbox. It intentionally excludes live command availability so compare mode tracks prerequisite-contract changes instead of machine-local PATH drift.
...
- `backend-operator-command-surface`: locks the restored backend operator-command route alongside the backend-owned `PUT /session/lifecycle-state` seam, and executes `text_question` plus `tts_preview` through the real route so both commands must round-trip back through the canonical `speech.lifecycle` snapshot for the current session character.
...
- `frontend-stage1-character-flow-runtime`: compiles a small Node TypeScript harness plus the extracted `useCharacterShellState` seam, feeds it the current backend Stage 1 snapshot, executes the real frontend bridge and active-character reconciliation logic, mounts the production prerequisite summary card into jsdom, and separately proves that the control shell still mounts that card so LLM, STT, and TTS rows plus visible state-and-blocker copy cannot drift back to source-only checks.

Run `npm run stability:control-summary` from `frontend/` to execute just this focused control-surface prerequisite-card guard without running the full stability suite.
- `frontend-shell-split-surface`: source-inspects the React entrypoints under `frontend/src/*.tsx` plus the app surfaces, recording whether every entrypoint still routes through `App`, whether backend sync, `speech.lifecycle`, and session-animation transport each stay isolated to their intended frontend owner seams, and whether operator commands stay isolated to one control-only client that publishes `text_question` and `tts_preview` only through the backend operator-command seam. Until real `/control` and `/display` entrypoints exist, this scenario records the dependency explicitly.
- `frontend-speech-lifecycle-runtime`: compiles a small Node TypeScript harness, proves the frontend loader preserves the canonical `speech.lifecycle` snapshot contract, proves the live-consumption seam appends the next cursor and updated synthesis envelope without inventing a second delivery contract, and verifies that backend artifact `audio_reference` values resolve to browser-playable frontend audio sources.
- `frontend-avatar-idle-default-runtime`: executes the backend animation service for the active character's session-idle command, then compiles a small Node TypeScript harness around the real frontend default-base path so drift back to a frontend-invented `idle.default` default fails when it no longer matches the backend-owned idle command.
- `frontend-punch-debug-runtime`: compiles a small Node TypeScript harness around `gesture.punch.once` and the real humanoid playback seam so the punch debug surface fails loudly if lower-arm quaternion diagnostics, representative upper-arm and hand bindings, or the dev-only debug bridge disappear.
