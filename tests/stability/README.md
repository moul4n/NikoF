...
- `frontend-shell-split-surface`: source-inspects the React entrypoints under `frontend/src/*.tsx` plus the app surfaces, recording whether every entrypoint still routes through `App`, whether `App.tsx` remains the sole owner of backend sync and `speech.lifecycle` consumption, and whether operator commands stay isolated to one control-only client that publishes `text_question` and `tts_preview` only through the backend operator-command seam. Until real `/control` and `/display` entrypoints exist, this scenario records the dependency explicitly.
