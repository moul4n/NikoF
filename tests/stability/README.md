...
Retired scenarios: `frontend-avatar-idle-default-runtime`, `frontend-punch-debug-runtime`, and `frontend-semantic-loop-assets-runtime` pinned the legacy mixer/quaternion-keyframe playback machinery (`humanoidChannelPlayback`, `avatarRuntimePlaybackRoute`, `baseAnimationMotionProfile`, `officialPunchClipPlayback`), which was removed when base animation playback consolidated onto the single official VRMA/Mixamo bridge (`frontend/src/avatar/runtime/animationPlayback.ts`).
