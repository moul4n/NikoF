...
- `backend-operator-command-surface`: locks the backend operator-command batch to one writable non-selection route plus two contract examples, proving text-question submission reuses the canonical transcription envelope, TTS preview reuses the canonical synthesis envelope, and both commands stay on the same backend-authored path.
