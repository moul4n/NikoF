...
- `frontend-shell-split-surface`: source-inspects `frontend/src/app/` and records which shell surface currently owns backend sync and `speech.lifecycle` consumption, so the control/display split can land without creating a second backend bridge or second speech lifecycle contract path. Until the split exists, this scenario records the dependency explicitly.
