...
- 2026-05-18T00:15:43+01:00: The first real operator preview slice can stay narrow by resolving Ollama and GPT-SoVITS only from `NIKOF_LLM_MODELS_ROOT`, `NIKOF_TTS_MODELS_ROOT`, and `NIKOF_PROVIDERS_ROOT`, with optional machine-local `runtime.json` overrides under those roots instead of new repo-tracked environment knobs.
- 2026-05-18T00:15:43+01:00: The default backend operator seam should use the real synthesis adapter when local roots are configured and degrade honestly to `unavailable` or `error` when they are not, rather than pretending the preview lane is healthy with stub-ready output.
- 2026-05-18T06:03:55Z: Keep the backend operator seam unchanged, but shape `text_question` prompts for speakable output and only synthesize when the LLM reply is actually ready; otherwise degraded LLM states will produce misleading voice output.
- 2026-05-18T06:03:55Z: Treat checked-in character `voice/profile.json` files as the repo-safe defaults and merge them with machine-local GPT-SoVITS `runtime.json` overrides under `NIKOF_TTS_MODELS_ROOT` or `NIKOF_PROVIDERS_ROOT` so speaker payloads and reference audio stay outside git.